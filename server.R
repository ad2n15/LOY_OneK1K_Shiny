...
shinyServer(function(input, output, session) { 
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  updateSelectizeInput(session, "sc1c1inp2", server = TRUE, 
   ...
   # === Download Handlers for .rds files ===

  output$download_b <- downloadHandler(
    filename = function() {
      "male_B_cells_191124_4.rds"
    },
    content = function(file) {
      file.copy("data/male_B_cells_191124_4.rds", file)
    }
  )

  output$download_cd4 <- downloadHandler(
    filename = function() {
      "male_CD4T_cells_191124_4.rds"
    },
    content = function(file) {
      file.copy("data/male_CD4T_cells_191124_4.rds", file)
    }
  )

  output$download_cd8 <- downloadHandler(
    filename = function() {
      "male_CD8T_cells_191124_4.rds"
    },
    content = function(file) {
      file.copy("data/male_CD8T_cells_191124_4.rds", file)
    }
  )

  output$download_mono <- downloadHandler(
    filename = function() {
      "male_mono_cells_191124_4.rds"
    },
    content = function(file) {
      file.copy("data/male_mono_cells_191124_4.rds", file)
    }
  )

  output$download_nk <- downloadHandler(
    filename = function() {
      "male_NK_cells_191124_4.rds"
    },
    content = function(file) {
      file.copy("data/male_NK_cells_191124_4.rds", file)
    }
  )
